//funzione che viene chiamata appena si carica l'album e genera le prime figurine
function loadAlbum(){
  ...
  //trovo l'album di figurine dell'utente
  ...
  //ho dovuto ciclare in tutta la mappa dato che figurine.size mi dava undefined
  var i=0
  for (const key in figurine) {
    i++
  }
  // Numero totale di elementi
  var totaleElementi = i

  // Calcolo del numero totale di pagine
  var numPagine = Math.ceil(totaleElementi / 20);

  // Genera il contenuto e la paginazione iniziale
  generaContenuto(1, figurine);
  generaPagine(numPagine);
}

//funzione che fa la chiamata API per un singolo personaggio
...
async function generaContenuto(pagina ,figurine) {
  // Questo è solo un esempio, puoi inserire il tuo contenuto qui
  var elementiPerPagina = 20; // Numero di elementi per pagina
  var inizio = (pagina - 1) * elementiPerPagina;
  var fine = inizio + elementiPerPagina;
  var i = 0 //variabile d'appoggio per stampare figurine tra inizio e fine

  var cardContainer = document.getElementById('contenuto');

  cardContainer.innerHTML = ''

  //ho deciso di creare diretto dinamicamente le colonne così ogni volta che l'utente cambia pagina posso cancellare il contenuto di 'contenuto' e ricreare la pagina template
  var cardTemplate = document.createElement('div');
  cardTemplate.className = 'row';
  cardTemplate.id = 'pg';
  cardTemplate.innerHTML = `
  <div id="card-pg" class="col-3 d-none">
  <div class="card" style="width: 18rem;">
      <h3 class="card-name">Card title</h3>
    <img src="..." class="card-img-top" alt="...">
    <div class="card-body">
      <p class="card-text"></p>
      <button class="btn btn-primary" onclick="ricordaIdCard(this.parentElement.parentElement.parentElement)">Visualizza dettagli</button>
    </div>
  </div>
  `;
  ...
  //ciclo tra le carte e controllo che siano comprese tra inizio e fine
  for (var key in figurine) {
    if(i>=inizio && i<fine){
      var character
      await getFromMarvel('public/characters', key).then(result => {
      ...
      clone.getElementsByClassName('card-name')[0].innerHTML 
      =character[0].name;

      clone.getElementsByClassName('card-text')[0].innerHTML 
      ="copie:"+figurine[key];

      clone.getElementsByClassName('card-img-top')[0].src = character[0].thumbnail.path+"."+character[0].thumbnail.extension;
      card.after(clone);
...
function generaPagine(numPagine) {
  var loggedUser = localStorage.getItem("loggedInUser") // Prendo l'utente corrente
  var albums = JSON.parse(localStorage.getItem('albums'))
  var figurine = new Map()
  for (var i = 0; i <  albums.length; i++) {
  ...
  for (var i = 1; i <= numPagine; i++) {
    var figurineString = JSON.stringify(Array.from(figurine)); // Converte la mappa in un array e lo serializza in JSON
    paginazione += "<li class='page-item' id='pagina" + i + "'><a class='page-link' href='#''>" + i + "</a></li>";
  }
  document.getElementById("paginazione").innerHTML = paginazione;

  for (var j = 1; j <= numPagine; j++) {
    document.getElementById("pagina" + j).addEventListener("click", function(event) {
        generaContenuto(parseInt(event.target.innerText), figurine);
    });
}
}

function ricordaIdCard(element) {

  var id = element.getAttribute("id")
  id = id.substr(id.length -7)
  console.log(id)
  window.location.href = 'InfoCarta.html?id=' + id
}
...
function mostraProfilo() {
  ...
  var daNascondere2 = document.getElementsByClassName('nav');
...
function reverse() {
  ...
  var daMostrare2 = document.getElementsByClassName('nav');
  var daNascondere = document.getElementsByClassName('hide');
