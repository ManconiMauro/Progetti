...
async function scegliCartaScambio() {
    var loggedUser = localStorage.getItem("loggedInUser") // Prendo l'utente corrente

    var scambi = JSON.parse(localStorage.getItem("scambi"))//prendo gli scambi salvati nel localStorage
    console.log(scambi)
    //controllo che ci siano scambi nel localStorage
    if (scambi != undefined) {
        //ciclo dentro a tutti gli scambi salvati per vedere se l'utente ha già uno scambio
        for (let i = 0; i < scambi.length; i++) {
            console.log(scambi[i])
            //controllo se lo scambio è stato concluso o meno
            if (scambi[i].mittente == loggedUser && scambi[i].destinatario == "") {
                //in questo caso non è stato concluseìo quindi lo mando alla scelta della carta da scambiare
                alert("Hai già inserito una carta che vorresti scambiare, verrai traferito alla pagina selezione")
                window.location.href = "Selezione.html"
            }else if(scambi[i].mittente == loggedUser){
                //in questo caso l'utente ha già inserito uno scambio, quindi gli do una notifica in cui gli chiedo se vuole eliminare o meno lo scambio già inserito
                Swal.fire({
                    title: 'Hai già scelto uno Scambio',
                    text: 'Sei sicuro di voler cancellare lo scambio già presente?',
                    icon: 'question',
    ...
    for (var i = 0; i <  albums.length; i++) {
        if(albums[i].user == loggedUser){
        figurine = albums[i].figurine
        break
        }
    }
    ...
    for (var id in figurine) {
       if (figurine[id] > 1 ) {
        console.log("carta trovata!") 
        var character
        await  getFromMarvel('public/characters', id)
            .then(result => character= result.data.results).catch(error => console.log(error));
        mostraCarte(character)
       }
   }
 }

 //funzione che prende la variabli scambi dentro al localStorage e che cancella lo scambio creato da quel determinato utente
 function eseguiAzione() {
    // In questo caso, mostriamo un messaggio di successo
    var loggedUser = localStorage.getItem("loggedInUser") // Prendo l'utente corrente
    var scambi = JSON.parse(localStorage.getItem("scambi"))
    console.log(scambi)
    for (let index = 0; index < scambi.length; index++) {
        if(scambi[index].mittente == loggedUser){
            scambi.splice(index, 1);
        }
    }
    localStorage.setItem('scambi', JSON.stringify(scambi))
    Swal.fire('Scambio rimosso!', 'L\'azione è stata eseguita con successo.');
  }

 function mostraCarte(character) {

    // Ottieni l'ID del personaggio dalla tua fonte dati
    var id = character[0].id;

    // Ottieni il riferimento alla carta iniziale
    var card = document.getElementById('card-pg');

    // Clona la carta iniziale
    var clone = card.cloneNode(true);

    // Imposta l'ID del clone con l'ID del personaggio
    clone.id = id;

    // Aggiorna il nome e l'immagine del personaggio nel clone
    clone.querySelector('.card-name').innerHTML = character[0].name;
    clone.querySelector('.card-img-top').src = character[0].thumbnail.path + ".jpg";

    // Inserisci il clone dopo la carta originale
    card.after(clone);

    // Rimuovi la classe 'd-none' per rendere visibile il clone
    clone.classList.remove('d-none');
}
 
function cartaScelta(personaggio){
    var loggedInUser = localStorage.getItem("loggedInUser") // Prendo l'utente corrente

    console.log(personaggio)

    // Ottieni l'id della carta clonata a cui appartiene il pulsante
    var cartaId = personaggio.id;

    // Ora puoi lavorare con l'id della carta clonata come desideri
    console.log("Id della carta clonata:", cartaId);

    // Creare una richiesta di scambio
    const richiestaScambio = {
        mittente: loggedInUser,
        cartaMittente: cartaId,
        destinatario: "",
        cartaDestinatario: "",
        accettato: false // Inizialmente la richiesta non è accettata
    };
...
function reverse() {
    var daMostrare = document.getElementsByClassName('centro');
    var daMostrare2 = document.getElementsByClassName('list-group');
