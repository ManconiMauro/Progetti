...
export default class CommunicationController {
    ...
    static async putUserSubscription(){
        const uid = await this.getUid();
        const sid = await this.getSid();
        let endpoint = "user/Febbraio1/"+uid+"/subscription/enable"
        let queryParams = {}
        let bodyParams = {sid: sid}
        console.log("putUserSubscription called with endpoint: ",endpoint, " and bodyParams:",bodyParams)
        return await this.genericPutRequest(endpoint,queryParams, bodyParams)
    }

    static async getUserFebbraio(){
        const uid = await this.getUid();
        const sid = await this.getSid();
        let endpoint = "user/Febbraio1/"+uid
        let queryParams = {sid: sid}
        console.log("getUser called with endpoint: ",endpoint, " and queryParams: ",queryParams)
        return await this.genericGetRequest(endpoint, queryParams)
    }

    static async getMenusFebbraio(lat, lng) {
        let endpoint = "menu/Febbraio1";
        const sid = await this.getSid();
        let queryParams = { lat: lat, lng: lng, sid: sid };
        let bodyParams = {};
        console.log(
