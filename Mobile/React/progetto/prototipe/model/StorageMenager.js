...
export default class StorageManager {
    ...
    async updateMenu(mid, imgVersion, base64){
        try {
            console.log(imgVersion)

            const query = 'UPDATE Dishes SET ImgVersion = ?, Base64 = ? WHERE Mid = ?';
    
            await this.db.runAsync(query, [imgVersion, base64, mid]);
            console.log('Menu Modificato con successo:', { mid, imgVersion});
        } catch (error) {
            // Gestione dell'errore
            if (error.message.includes('UNIQUE constraint failed')) {
                console.error('Errore: Mid già presente nel database.');
    ...
    async getAllMenus(){
        try {
            ...
            if (result) {
                console.log('Tuttu i menu trovati:', result);
                return result;
            } else {
                console.warn('Nessun menu trovato');
                return null;
            }
        } catch (error) {
            console.error('Errore durante il recupero del menu:', error);
