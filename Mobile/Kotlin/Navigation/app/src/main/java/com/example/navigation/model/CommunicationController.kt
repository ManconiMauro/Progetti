...
import com.example.navigation.database.SIDManager
...
object CommunicationController {
    ...
    suspend fun putUser(user: UserToSave): UserResponse? {
        ...
        return try {
            ...
            if (httpResponse.status.value in 200..299) {
                // Se la risposta è 204, non fare il parsing del corpo
                if (httpResponse.status.value == 204) {
                    Log.d(TAG, "No content returned (204), but request was successful.")
                    return null // Non c'è nulla da parsare, ma l'operazione è riuscita
                }
                ...
                // Se il codice di stato non è tra 200 e 299, trattalo come errore
                Log.e(TAG, "Request failed with status: ${httpResponse.status.value}")
                return null
            }
        } catch (e: Exception) {
